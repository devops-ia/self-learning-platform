...
function handlePlan(code: string) {
  const hasDependsOn = code.includes("depends_on");
...
function handleGraph(code: string) {
  const hasDependsOn = code.includes("depends_on");
  if (!hasDependsOn) {
    return {
      output: [
...
function validateDependsOnExists(code: string) {
  if (!code.includes("depends_on")) {
    return {
      passed: false,
      errorMessage: [
...
function validateDependsOnDatabase(code: string) {
...
function validateDependsOnSyntax(code: string) {
...
export const missingDepends: Exercise = {
